{ pkgs, ... }:
{
  extraConfigLuaPre = ''
    ...
    local java_debug_bundle = vim.split(vim.fn.glob("${pkgs.khanelivim.java-debug}" .. "/*.jar"), "\n")
    local java_test_bundle = vim.split(vim.fn.glob("${pkgs.khanelivim.java-test}" .. "/*.jar", true), "\n")
      ...
      settings = {
        java = {
          ...
          format = {
            enabled = true;
            settings = {
              url = "${pkgs.khanelivim.eclipse-java-google-style}";
              profile = "GoogleStyle";
            };
          };
