      ...
      callback.__raw = ''
        function(args)
          local function apply_rust_maps(bufnr)
            ...
            -- RUN / DEBUG
            map("n", "<leader>zr", "runnables", "Rust Runnables")
            map("n", "<leader>zd", "debuggables", "Rust Debuggables")
            -- map("n", "<leader>RR", "run", "Run target at cursor")
            -- map("n", "<leader>DD", "debug", "Debug target at cursor")
            map("n", "<leader>zt", "testables", "Rust Testables")
            ...
            map({ "n", "x" }, "<leader>za", "codeAction", "Code Action (Rust)")
            map("n", "<leader>zh", "hover actions", "Hover Actions (Rust)")
            map("x", "<leader>zh", "hover range", "Hover Range (Rust)")
            map("n", "<leader>zm", "expandMacro", "Expand Macro")
            map("n", "<leader>zp", "rebuildProcMacros", "Rebuild Proc Macros")
            map("n", "<leader>zj", "joinLines", "Join Lines")
            map("x", "<leader>zj", "joinLines", "Join Lines (Selection)")
            -- map("n", "<leader>lu", "moveItem up", "Move Item Up")
            -- map("n", "<leader>ld", "moveItem down", "Move Item Down")
            map("n", "<leader>zs", "ssr", "Structural Search/Replace")
            map("x", "<leader>zs", "ssr", "Structural Search/Replace (Selection)")
            ...
            map("n", "<leader>zq", "renderDiagnostic", "Render Diagnostic (Cycle)")
            map("n", "<leader>ze", "renderDiagnostic current", "Render Diagnostic (Current Line)")
            map("n", "<leader>zx", "explainError current", "Explain Error (Current Line)")
            -- map("n", "<leader>lr", "relatedDiagnostics", "Related Diagnostics")

            -- NAVIGATION / DOCS / WORKSPACE
            map("n", "<leader>zo", "openDocs", "Open Docs (Rust)")
            map("n", "<leader>zc", "openCargo", "Open Cargo.toml")
            map("n", "<leader>zg", "crateGraph", "Crate Graph")
            map("n", "<leader>zP", "parentModule", "Open Parent Module")
            ...
            map("n", "<leader>zW", "reloadWorkspace", "Reload Workspace")
            -- map("n", "<leader>lY", "syntaxTree", "Syntax Tree")
            -- map("n", "<leader>lV", "view hir", "View HIR")
            -- map("n", "<leader>lN", "view mir", "View MIR")
            map("n", "<leader>zO", "externalDocs", "Open External Docs")
            map("n", "<leader>zl", "logFile", "Open rust-analyzer Log")
          end
