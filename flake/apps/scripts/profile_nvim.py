...
def build_command_for_profile(profile):
    ...
    return f"nix build --impure --no-link --print-out-paths --expr '{expr}'"
...
def run_profile(nvim_bin, output_path, event="ui", interactive=False):
    ...
    if interactive:
        ...
        console.print("[dim]Starting nvim (will auto-quit after profiling)...[/dim]")
        result = subprocess.run(
            [str(nvim_bin)],
            env=merged_env,
        )
...
def average_profiles(profiles):
    ...
    startup_times = [
        p.get("startup_time_ms", 0) for p in profiles if p.get("startup_time_ms")
    ]
    if startup_times:
        avg["startup_time_ms"] = sum(startup_times) / len(startup_times)

    # Average trace times
    trace_times = [p.get("total_trace_time_ms", 0) for p in profiles]
    ...
    for profile in profiles:
        for plugin in profile.get("by_plugin", []):
            ...
            plugin_data[name]["times"].append(plugin.get("time_ms", 0))
            plugin_data[name]["counts"].append(plugin.get("count", 0))
...
def print_profile_summary(profile, title="Profile Summary"):
    ...
    if profile.get("startup_time_ms"):
        console.print(
            f"Startup time: [green]{profile['startup_time_ms']:.2f}ms[/green]"
        )
    console.print(f"Event: {profile.get('event', 'unknown')}")
    console.print(f"Total trace time: {profile.get('total_trace_time_ms', 0):.2f}ms")
...
def print_comparison(diff):
    ...
    console.print(
        f"Delta:    [{color}]{sign}{delta_ms:.2f}ms ({sign}{delta_pct:.1f}%)[/{color}]"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of profiling iterations (default: 10 for baseline, 5 otherwise)",
    )
    parser.add_argument(
        "-i",
        "--interactive",
    ...
    if len(profiles) == 0:
        console.print("[red]No successful profiles captured[/red]")
        console.print(
    ...
    if len(profiles) == 1:
        ...
        console.print(
            f"\n[dim]Discarding first run, averaging {len(profiles) - 1} runs...[/dim]"
        )
    ...
    if args.baseline:
        ...
        with open(baseline_file, "w") as f:
            json.dump(averaged, f, indent=2)
        console.print(f"[green]Baseline saved:[/green] {baseline_file}")
        ...
        if baseline_file.exists():
            ...
            if baseline:
                ...
                if diff:
                    ...
                    # Save diff
                    diff_path = output_dir / f"diff-{profile}-{timestamp}.json"
                    with open(diff_path, "w") as f:
                        json.dump(diff, f, indent=2)
                    console.print(f"[green]Diff saved:[/green] {diff_path}")
        else:
            console.print(
